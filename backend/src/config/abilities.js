...
const { ROLES, MODULOS } = require('../utils/constants');
 ...
 * @param {string} usuario.usuario_id - ID del usuario
 * @param {string} usuario.rol - Rol del usuario
 * @param {boolean} usuario.activo - Si el usuario está activo
 * @param {number} usuario.empleado_id - ID del empleado (si aplica)
 * @returns {Ability} - Objeto de habilidades CASL
 */
function definirAbilidades(usuario) {
    ...
    if (usuario.rol === ROLES.CAJERO) {
        ...
        // Puede actualizar solo ciertos estados de pedido
        can('update', MODULOS.PEDIDO, ['estado', 'observaciones', 'total', 'metodo_pago']);
        ...
        // Inventario (solo lectura para verificar disponibilidad)
    ...
    if (usuario.rol === ROLES.MESERO) {
        ...
        // Puede actualizar solo pedidos en estado pendiente (campo observaciones únicamente)
    ...
    if (usuario.rol === ROLES.COCINERO) {
        // Pedidos (ver solo pedidos en preparación)
        can('read', MODULOS.PEDIDO);
        can('read', MODULOS.DETALLE_PEDIDO);
        
        // Puede actualizar solo el estado de pedidos en preparación
        can('update', MODULOS.PEDIDO, ['estado']);
        
        // Productos (ver para consultar recetas)
        can('read', MODULOS.PRODUCTO);
    ...
    // Los usuarios pueden cambiar su propia contraseña a través de endpoints específicos
 ...
 */
function verificarPermiso(usuario, accion, sujeto, condiciones = {}) {
